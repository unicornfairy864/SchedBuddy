...
### Changed（设计定版：本地优先模型 + 自动发现 + 防火墙自动放行 · 纯文档/决策，无代码变更）
- **1. 本地优先（离线独立）**：App 内置库为本地权威副本，无电脑也可全功能使用；桌面/网页降级为可选同步对端；同步改**双向**手动 pull/push/merge（原"唯一权威主机"模型作废）。
- **2. 局域网自动发现**：主机 mDNS 公告（`_schedbuddy._tcp.local`:3876）+ UDP 广播辅助；App 自动扫描列出主机，回退扫二维码 / 手动 IP（Android 本地网络权限）。
- **3. Windows 防火墙自动放行**：NSIS 安装器（提权）或首启一次 UAC 自动建入站规则（TCP 3876），免用户手动开端口；提权受限时回退手动并提示。
- 文档同步：`00-decisions.md`（§1 行 1/4/8–11 修订 + §5 ADR 三行）、`项目说明.md`（规则 1/6 与路线表 v0.5/v0.5+）、`01-architecture.md`（§1/§1.1 拓扑与 §5）、`04-api.md`（发现与防火墙节）、README。
...
- 实机确认手机竖屏下「纵向日卡列表」适读 → **取消**移动端「横向时间轴 + packDay 排布」移植计划（原 0.4.4 第 2 步）；Web/桌面维持横向时间轴不变。
- 决策登记 `00-decisions.md`（ADR-lite，v0.3.17）；`项目说明.md` 规则 8 / v0.4 进度 / 路线表、`01-architecture.md` §5 与元信息同步（横向时间轴口径标注为仅适用 Web/桌面）。

## [0.3.16] - 2026-09-08

### Added（v0.4 移动端 0.4.4 第 1 步：只读周/日视图 + 双端日历）
- **mobile 只读视图**：启动即连主机拉取日程；周视图（周一起 7 天日卡，今天高亮）+ 日视图，‹ 今天 › 翻页；数据经 shared `expandAll` 本地展开（同源算法）；点击日程展开详情显示**备注**；顶条日期不再显示「第 N 周」。
- **双端日期标题 → 圆角胶囊钮**：Web（Header `.range-btn`）与移动端均改为按钮形态，点击弹出**日历弹层**：周视图 = **周历**（当前周由**一个深绿圆角框**整行包住 7 天，框内今天为实心圆点、非粗体），日视图 = **日历**（选中单日为圆点）；月份可前后翻、今天以颜色标识（非粗体）。
- mobile `src/api.ts` 新增 `fetchSchedules`。
- 验收驱动微调多轮（深绿整周框、去 7 方块、去今日粗体）；APK 与 Web 已随附更新。
