...
> 状态：生效　·　最近更新：v0.3.9（2026-09-06）　·　规范：`07-doc-standards.md`
> 范围：仅 **A1（RFC 5545 本体）+ A2（RFC 6868 参数转义）**；CalDAV（4791）、iTIP/iMIP（5546/5547）、xCal/jCal（6321/7265）不在范围。
> UI（设置 → 数据管理 → 导出/导入 .ics）入口方案待用户另行给出；本文件先固化引擎与映射口径。
...
## 4. 边界与未来（不在本期）
- 导入 .ics 的“合并/覆盖”语义与 JSON 导入一致（同 group=同 id 覆盖）——待 UI 阶段接线。
