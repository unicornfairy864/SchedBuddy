...
**现状（v0.4 起，2026-09-07 实现）**：服务绑定 `0.0.0.0`。对写请求（POST/PUT/DELETE）判定：

- **回环地址**（`127.0.0.1` / `::1`，桌面主机本机）→ 允许，writer=`'pc'`；
- **携带有效设备 token**（`Authorization: Bearer <token>`，经 `POST /api/pair` PIN 一次性配对换发）→ 允许，writer=`deviceId`；
- 带**无效/已撤销** token → `401 unauthorized`；
- 其余（未配对局域网客户端，含手机浏览器）→ `403 readonly`。
- GET 一律放行。

PIN 由主机端「设置 → 移动设备配对」生成（仅回环可调 `POST /api/pin`），**单次、10 分钟有效**（服务重启作废）；token 长期有效，可在主机「设置」中撤销（撤销后立即 `401`）。
...
| POST | `/api/schedules` | 新建；body: Schedule 对象（不含 id/created_at/updated_at 及同步元字段）；`force?:boolean`。错误：`{error:'conflict', conflicts:[...]}` 或 `{error:'invalid', issues:[...]}`。新建 = rev 1、lastWriter `'pc'` 或设备 id |
...
| POST | `/api/pin` | （仅回环）生成一次性配对 PIN → `{ pin, expiresAt }`；单次、10 分钟有效 |
| POST | `/api/pair` | PIN 配对：`body { pin, name? }` → `201 { deviceId, token }`；PIN 错误/过期 → `403 badpin` |
| GET | `/api/devices` | （仅回环）已配设备列表 `{ devices: [{ deviceId, name, createdAt, lastSeenAt }] }`（不返回 token） |
| DELETE | `/api/devices/:deviceId` | （仅回环）撤销设备（物理删除，token 立即失效）；不存在 → `404` |
...
## 规划扩展接口（已决策 · v0.5 实现；当前代码未含）

面向 Android App 的**离线同步**接口（v0.5：pull/push/merge）。Base 同 `/api`；写类均校验设备 token（配对接口 `/api/pin` `/api/pair` `/api/devices` 已随 v0.4 实现，见上表）。
...
错误：token 缺失/无效 → `401 { error:'unauthorized' }`；`baseRev` 落后（期间主机已前移）→ `409 { error:'conflict', baseRev, currentRev }` 需重拉再推。
...
| `readonly` | 403 | 未授权客户端写入被拒（现状 = 未配对 LAN；仅回环与有效 token 可写） |
| `unauthorized` | 401 | 写请求带无效/已撤销 token（v0.4 已实现） |
| `badpin` | 403 | 配对 PIN 错误/过期（v0.4 已实现） |
| `notfound` | 404 | id 不存在（PUT/DELETE 日程、撤销设备） |
