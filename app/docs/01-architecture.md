...
- **0.1**（当前）：仓库/文档/骨架 + git。
- **0.2**：shared 规则引擎与排布算法（含单测）；server API + SQLite + 备份；web 周/日视图只读渲染。
- **0.3**：创建/编辑/删除表单（颜色取色、时段列表、规则表单）+ 冲突检测告警 + 动画打磨。
- **0.4**：desktop 壳内嵌服务 + 本地窗口 + LAN 只读验证 + 单次跳过/改期。
- **0.5+**：导出/导入、设置（学期起点/校历）、月视图、安装包分发（需授权确认）。
