...
- **0.4**：**Android App（RN + TS）骨架**：`app/mobile` workspace、局域网直连主机、周/日**只读视图**（复用 shared 展开/日期引擎；**RN 纵向日卡列表**，不移植横向时间轴 —— `03-ui-spec` 的横向时间轴口径仅适用 Web/桌面）、主机地址配置 + **PIN 一次性配对换发设备 token**（server 增加配对端点与 token 写校验）。
